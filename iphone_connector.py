...
class IPhoneConnector(object):
    ...
    def __get_message_id_joined_to_phone_or_email(self):
        ...
        WHERE
          ...
          AND ((chat_handle_join.handle_id = message.handle_id) OR message.is_from_me)''',
            self._message_con)

        # Clean it up a bit.
        message_id_joined_to_phone_or_email[
            'country'] = message_id_joined_to_phone_or_email[
                'country'].str.lower()
        message_id_joined_to_phone_or_email[
            'phone_or_email'] = message_id_joined_to_phone_or_email.apply(
                self.__standardize_phone_numbers, axis=1)
    ...
    def __get_address_joined_with_message_id(self, address_book):
        address_joined_with_message_id = (
            self.__get_message_id_joined_to_phone_or_email().merge(
                address_book,
                how='left',
                left_on='phone_or_email',
                right_index=True,
                indicator='merge_chat_with_address'))
        address_joined_with_message_id = address_joined_with_message_id.drop(
            ['ROWID'], axis=1)

        key_fields = ['message_id', 'chat_id', 'phone_or_email']
        duplicates = address_joined_with_message_id.duplicated(
            subset=key_fields, keep=False)
        if not sum(duplicates) == 0:
            error_message = (
                'WARNING: tuple (message_id, chat_id, and phone_or_email) '
            ...
            address_joined_with_message_id.drop_duplicates(
                subset=key_fields, inplace=True)
    ...
    def __init__(self):
        ...
        self._latest_sync_dir = self.__get_latest_dir_in_dir(base_dir)
        print('Latest iPhone backup directory: {0}'.format(
            self._latest_sync_dir))

        # Newer iPhone OS's shard the backup into subdirectories starting with the first two chars
        # of the files within them.
        has_subdirectory_structure = MESSAGE_DB[:2] in os.listdir(
            self._latest_sync_dir)
        if has_subdirectory_structure:
            message_path = os.path.join(self._latest_sync_dir, MESSAGE_DB[:2],
                                        MESSAGE_DB)
            address_path = os.path.join(self._latest_sync_dir, ADDRESS_DB[:2],
                                        ADDRESS_DB)
        else:
            message_path = os.path.join(self._latest_sync_dir, MESSAGE_DB)
            address_path = os.path.join(self._latest_sync_dir, ADDRESS_DB)

        self._message_con = sqlite3.connect(message_path)
        self._address_con = sqlite3.connect(address_path)
    ...
    def get_message_df(self):
        ...
        messages_df = pd.read_sql_query('''
          SELECT
            ...
            has_dd_results
          FROM message''', self._message_con)
        ...
        messages_df['date_delivered'] = pd.to_datetime(
            messages_df['date_delivered'])
    ...
    def get_address_book(self):
        ...
        # Clean it up a bit.
        address_book = address_book[(address_book['property'] == 4) | (
            address_book['property'] == 3)]  # Of type phone or email
        address_book['phone_or_email'] = address_book.apply(
            self.__standardize_phone_numbers, axis=1)

        # Convert a few columns to dates.
        address_book['birthday'] = pd.to_datetime(
            address_book['birthday'], errors='coerce')
        address_book['creation_date'] = pd.to_datetime(
            address_book['creation_date'])
        address_book['modification_date'] = pd.to_datetime(
            address_book['modification_date'])
    ...
    def get_merged_message_df(self,
                              messages_df,
                              address_book,
                              print_debug=False):
        """
        ...
        """
        phones_with_message_id_df = self.__get_address_joined_with_message_id(
            address_book)
        ...
        return messages_df.merge(
            phones_with_message_id_df,
            how='left',
    ...
    def _collapse_first_last_company_columns(self, df):
        ...
        def create_full_name(row):
            atoms = [
                atom for atom in [row['first'], row['last'], row['company']]
                if atom
            ]
            atoms_as_str = [
                atom.encode('utf8')
                if type(atom).__name__ == 'unicode' else str(atom)
                for atom in atoms
            ]
    ...
    def get_cleaned_fully_merged_messages(self):
        ...
        messages_df = self.get_message_df()
        # Drop some columns that we don't use now, but may in the future.
        messages_df.drop(
            [
                'version', 'is_emote', 'is_read', 'is_system_message',
        ...
        address_book_df = self.get_address_book()
        ...
        fully_merged_messages_df = self.get_merged_message_df(
            messages_df, address_book_df)
        # Drop a few columns we don't care about for now
        fully_merged_messages_df = fully_merged_messages_df.drop(
            [
                'handle_id', 'country_messages_df', 'country_other_join_tbl',
                'service_messages_df', 'service_other_join_tbl'
            ],
            axis=1)

        print('Messages with phone numbers not found in address book: {0:,}'.
              format(fully_merged_messages_df[
                  fully_merged_messages_df.merge_chat_with_address != 'both']
                     .shape[0]))

        print((
            'Messages loaded: {0:,} (this is larger than the length of the messages table due to group '
            'messages you sent)').format(fully_merged_messages_df.shape[0]))

        # Drop some columns that we're no longer going to need.
        fully_merged_messages_df = fully_merged_messages_df.drop(
            [
                'merge_chat_with_address',
        ...
        self._collapse_first_last_company_columns(fully_merged_messages_df)
        self._collapse_first_last_company_columns(address_book_df)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Print out the text messages and contacts from '
    ...
    parser.add_argument(
        ...
        dest='full',
        help=
        'If passed, message output includes more than just the text, date and '
    ...
    message_df, addresses_df = connector.get_cleaned_fully_merged_messages()
    # Note we don't explicitly print phone_or_email since it's the index.
    addresses_to_print = addresses_df if args.full else addresses_df[[
        'full_name'
    ]]
    messages_to_print = message_df if args.full else message_df[[
        'full_name', 'date', 'text'
    ]]

    if args.output_directory:
        addresses_to_print.to_csv(
            os.path.join(args.output_directory, 'addresses.csv'),
            encoding='utf-8')
        messages_to_print.to_csv(
            os.path.join(args.output_directory, 'messages.csv'),
            encoding='utf-8')
    else:
